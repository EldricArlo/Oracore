...
int test_verification_successful_path() {
    ...
    // [修正] 使用种子 0xAA 生成 CA
    if (generate_test_ca(&ca_key, &ca_cert, 0xAA) != 0) {
        fprintf(stderr, "TEST FAILED: CA generation failed (%s:%d)\n", __FILE__, __LINE__);
        goto cleanup;
    }
    if (generate_master_key_pair(&user_kp) != 0) {
        fprintf(stderr, "TEST FAILED: User keygen failed (%s:%d)\n", __FILE__, __LINE__);
        goto cleanup;
    }
    if (generate_csr(&user_kp, username, &user_csr) != 0) {
        fprintf(stderr, "TEST FAILED: CSR gen failed (%s:%d)\n", __FILE__, __LINE__);
        goto cleanup;
    }
    if (sign_csr_with_ca_timed(&user_cert, user_csr, ca_key, ca_cert, 0, 31536000L) != 0) {
        fprintf(stderr, "TEST FAILED: Cert signing failed (%s:%d)\n", __FILE__, __LINE__);
        goto cleanup;
    }

    int res = verify_user_certificate(user_cert, ca_cert, username);
    if (res != -4) {
        fprintf(stderr, "TEST FAILED: Verification must fail with OCSP error (-4) for non-existent responder, but got %d (%s:%d)\n", res, __FILE__, __LINE__);
        goto cleanup;
    }
...
int test_verification_untrusted_ca() {
    ...
    // [修正] 使用不同的种子生成两个不同的CA
    if (generate_test_ca(&ca1_key, &ca1_cert, 0xAA) != 0) goto cleanup; // Trusted CA
    if (generate_test_ca(&ca2_key, &ca2_cert, 0xBB) != 0) goto cleanup; // Untrusted CA
...
int test_verification_subject_mismatch() {
    ...
    // [修正] 使用种子 0xCC 生成 CA
    if (generate_test_ca(&ca_key, &ca_cert, 0xCC) != 0) goto cleanup;
    if (generate_master_key_pair(&user_kp) != 0) goto cleanup;
