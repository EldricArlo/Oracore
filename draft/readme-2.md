...
2.  **运行单元测试 (关键步骤):**
    ...
    > **关于 OCSP 测试的预期行为**
    >
    > 您可能会注意到 `test_pki_handler` 的一个测试用例似乎“失败”了，并打印出 OCSP 相关的错误。**这是完全符合预期的！**
    >
    > 为了验证我们的“故障关闭”安全策略，`test_certificate_validation_successful` 测试用例会故意使用一个指向无效 OCSP 服务器 (`http://127.0.0.1/...`) 的证书进行验证。由于网络请求必然失败，`verify_user_certificate` 函数必须返回 `-4` 以表示吊销状态检查失败。测试代码会断言返回值确实是 `-4`，如果断言成功，则证明我们的安全机制工作正常。
...
2.  **Alice 和 Bob 各自生成 CSR 并提交给 CA 以获取证书。** (此处我们假设 CA 已签发 `alice.pem` 和 `bob.pem`)
    ```bash
    # Alice 的操作
...
3.  **Alice 在发送文件前，必须先验证 Bob 的证书的有效性。** (假设 `ca.pem` 是受信任的根 CA 证书)
    ```bash
    ./bin/hsc_cli verify-cert bob.pem --ca ca.pem --user "bob@example.com"
    # 如果一切正常，将输出成功信息
    ```

4.  **验证通过后，Alice 加密她的机密文件 `secret.txt` 给 Bob:**
    ```bash
    # 创建一个示例文档
...
1. 生成一次性密钥
   [Session Key]
       ...
       |-------------+
       v             |
 [对称加密 (AEAD)]   |   [非对称加密 (Key Encapsulation)]
       |             |           |
       v             |           |  + Bob 的公钥 (from bob.pem)
 [加密数据]          |           |  + Alice 的私钥 (from alice.key)
       |             |           v
       |             +------> [封装后的会話密钥]
       |                           |
       |                           |
      ...
      (包含: 加密数据 + 封装后的密钥)
                     ...
                     |                           |
                     |                           v
                     |                 [非对称解密 (Key Decapsulation)]
                     |                           |
                     |                           | + Alice 的公钥 (from alice.pem)
                     |                           | + Bob 的私钥 (from bob.key)
                     |                           v
                     |                       [Session Key] (恢复)
                     |                           |
                     +---------------------------+
